...
import { emailSignInStart, googleSignInStart, githubSignInStart } from "../../redux/auth/auth.actions";
	...
	return (
		...
		>
			...
			<motion.button
				type="button"
				variants={authFadeInUpVariants}
				className={`SignIn__form--button button__github ${isLoading && 'loading'}`}
				disabled={isLoading}
				onClick={() => dispatch(githubSignInStart())}
			>
				{!isLoading && <GithubOutlined />}
				{isLoading ? <Loader /> : 'Sign in with Github'}
			</motion.button>
		</motion.form>
	)
}
